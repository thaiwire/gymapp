...
function DepartmentsPage() {
  ...
  const countLabel = useMemo(() => {
    if (departments.length === 1) return "1 department";
    return `${departments.length} departments`;
  }, [departments.length]);

  const mapToDrafts = (rows: DepartmentRow[]) =>
    rows.reduce<Record<string, DepartmentDraft>>((acc, row) => {
      acc[row.id] = {
        name: row.name ?? "",
  ...
  const loadDepartments = async () => {
    ...
    if (!result.success) {
      toast.error(result.error || "Unable to fetch departments.");
      setDepartments([]);
      setDrafts({});
      setIsLoading(false);
      return;
  ...
  const handleSave = async (id: string) => {
    ...
    if (!result.success) {
      toast.error(result.error || "Unable to update department.");
      return;
    }
  ...
  const handleDelete = async (id: string) => {
    ...
    if (!result.success) {
      toast.error(result.error || "Unable to delete department.");
      return;
    }
    ...
    <section className="space-y-6">
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <h1 className="text-xl font-semibold text-zinc-800">Manage Departments</h1>
            ...
            <tbody>
              {isLoading ? (
                ...
                departments.map((dept) => {
                  ...
                  return (
                    <tr key={dept.id} className="border-b border-zinc-100 align-top">
                      <td className="px-3 py-3">
                        <Input
                          value={draft.name}
                          onChange={(e) => updateDraft(dept.id, "name", e.target.value)}
                          placeholder="Name"
                          className="h-9"
                        />
                      </td>
                      ...
                      <td className="px-3 py-3">
                        <Input
                          ...
                          onChange={(e) =>
                            updateDraft(dept.id, "description", e.target.value)
                          }
                      ...
                      <td className="px-3 py-3">
                        <Select
                          ...
                          onValueChange={(value) =>
                            updateDraft(dept.id, "is_active", value === "active")
                          }
                        >
